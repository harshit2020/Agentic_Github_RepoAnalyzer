...
def create_connect_collection_api(user_id): #change this to get this from redis
    try:
        r = connect_redis_store()
        exists = r.exists(user_id)
        if exists == 0:
            raise RuntimeError(f"Chroma credentials does not exist in redis.")
        
        user_id = r.hgetall(user_id)
        CHROMA_HOST = user_id["CHROMA_HOST"]
        CHROMA_API_KEY = user_id["CHROMA_API_KEY"]
        CHROMA_TENANT = user_id["CHROMA_TENANT"]
        CHROMA_DATABASE = user_id["CHROMA_DATABASE"]
...
def create_connect_collection_localhost():
    try:
...
if __name__ == "__main__":
    ...
    create_connect_collection_api("test_mail@gmail.com")

...
def chunk_function(source_code:str,chunk_types:set,tokenizer,parser:Parser,filename,path,embedding_MAX_TOKEN):
    try:
        ...
        def walk(node):
            if node.type in chunk_types:
                ...
                token_count = len(tokenizer.encode(chunk_node))
                without_code_string = len(tokenizer.encode("{code:,filename:,filepath:}"))
                tokens_count_path = len(tokenizer.encode(path))
                tokens_count_filename = len(tokenizer.encode(filename))
                token_count_per_chunk = token_count  + tokens_count_path + tokens_count_filename
                if token_count <= embedding_MAX_TOKEN:
                    chunks.append({"code":chunk_node,"tokens":token_count_per_chunk,"filename":filename,"filepath":path})
                else:
...
def chunk_acc_file_type(path,tokenizer,embedding_MAX_TOKEN):
    ...
    for doc in docs:
        ...
        if ext == ".py":
            py_chunks = chunk_function(source_code,PY_CHUNK_TYPES,tokenizer,PY_PARSER,filename,path,embedding_MAX_TOKEN)
            source_code_chunks.extend(py_chunks)
        elif ext in {".js",".jsx"}:
            js_chunks = chunk_function(source_code,JS_CHUNK_TYPES,tokenizer,JS_PARSER,filename,path,embedding_MAX_TOKEN)
